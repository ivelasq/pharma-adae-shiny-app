...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    checkboxGroupInput("severity_filter", "Filter by Severity",
                       choices = unique(adae_data$AESEV),
                       selected = unique(adae_data$AESEV)),
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$severity_filter)) {
      data <- data %>% filter(adae_data$AESEV %in% input$severity_filter)
    }
    
    if (input$serious_only) {
      data <- data %>% filter(adae_data$AESER == "Y")
    }
